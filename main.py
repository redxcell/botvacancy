...
async def cmd_new_ad(message: Message, state: FSMContext):
    ...
    if not is_subscribed:
        await message.answer(
            ...
            f"После подписки отправьте команду /new снова.",
            parse_mode="HTML",
            reply_markup=ReplyKeyboardRemove()
        )
        return
    
    # Начинаем процесс создания объявления
...
async def process_ad_text(message: Message, state: FSMContext):
    ...
    if check_banned_words(ad_text):
        ...
        await message.answer(
            ...
            f"Для создания нового объявления отправьте /new\n\n"
    ...
    try:
        ...
        await message.answer(
            ...
            f"Чтобы разместить ещё одно объявление, отправьте /new",
            parse_mode="HTML",
            reply_markup=ReplyKeyboardRemove()
        )
...
async def handle_media(message: Message):
    ...
    await message.answer(
        "❓ Для размещения объявления отправьте команду /new\n\n"
    ...
    if was_not_member and is_now_member:
        try:
            welcome_message = (
                ...
                f"📋 Правила: отправьте боту команду /rules\n\n"
